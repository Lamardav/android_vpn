...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (VPN_SERVICE_ACTION.equals(action) || !DISCONNECT_ACTION.equals(action)) {
                ...
                if (bundle != null) {
                    ...
                    // Global settings
                    int mtu = bundle.getInt("MTU", 1400);
                    Log.d(TAG, "MTU = " + mtu);
                    profile.setMTU(mtu);
                    ...
                    // VPN type
                    String vpnTypeId = bundle.getString("VpnType");
                    Log.d(TAG, "VpnType = " + vpnTypeId);
                    profile.setVpnType(VpnType.fromIdentifier(vpnTypeId));

                    if ("ikev2-p12".equals(vpnTypeId)) {
                        // Load .p12 from assets and extract cert/key
                        Log.d(TAG, "Using P12 authentication");
                        try {
                            InputStream p12Stream = getAssets().open("client1.p12");
                            char[] pwd = bundle.getString("P12Password").toCharArray();
                            Log.d(TAG, "P12 password length = " + pwd.length);
                            KeyStore ks = KeyStore.getInstance("PKCS12");
                            ks.load(p12Stream, pwd);

                            Enumeration<String> aliases = ks.aliases();
                            if (!aliases.hasMoreElements()) {
                                throw new IllegalStateException("PKCS12 is empty");
                            }
                            String alias = aliases.nextElement();
                            Log.d(TAG, "P12 alias = " + alias);

                            X509Certificate cert = (X509Certificate) ks.getCertificate(alias);
                            PrivateKey key = (PrivateKey) ks.getKey(alias, pwd);
                            Certificate[] chain = ks.getCertificateChain(alias);
                            Log.d(TAG, "Certificate and key extracted, chain length = " + (chain != null ? chain.length : 0));

                            File certFile = saveCertPem("client-cert.pem", cert.getEncoded());
                            Log.d(TAG, "Saved client-cert.pem to " + certFile.getAbsolutePath());
                            File keyFile  = saveKeyPem("client-key.pem", key);
                            Log.d(TAG, "Saved client-key.pem to " + keyFile.getAbsolutePath());
                            File caFile = null;
                            if (chain != null && chain.length > 1) {
                                caFile = saveCertPem("ca-cert.pem", ((X509Certificate) chain[1]).getEncoded());
                                Log.d(TAG, "Saved ca-cert.pem to " + caFile.getAbsolutePath());
                            }

                            profile.setCertificateAlias(caFile != null ? caFile.getAbsolutePath() : null);
                            profile.setUserCertificateAlias(certFile.getAbsolutePath());
                            profile.setLocalId(keyFile.getAbsolutePath());
                        } catch (Exception e) {
                            Log.e(TAG, "P12 loading failed", e);
                            profile = null;
                        }
                    } else {
                        // EAP authentication
                        Log.d(TAG, "Using EAP authentication");
                    ...
                    profile.setFlags(0);
                    retry = bundle.getBoolean(KEY_IS_RETRY, false);
                    Log.d(TAG, "Retry flag = " + retry);
                }
            }
            if (profile != null && !retry) {
                Log.d(TAG, "Deleting old log file: " + LOG_FILE);
                deleteFile(LOG_FILE);
            }
            Log.d(TAG, "setNextProfile(" + profile + ")");
...
//        builder.setContentIntent(pending);
    ...
    private File saveCertPem(String name, byte[] der) throws IOException {
        String pem = "-----BEGIN CERTIFICATE-----\n"
                + android.util.Base64.encodeToString(der, android.util.Base64.NO_WRAP)
                + "\n-----END CERTIFICATE-----\n";
        File file = new File(getCacheDir(), name);
        try (FileOutputStream fos = new FileOutputStream(file)) {
            fos.write(pem.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        }
        return file;
    }

    private File saveKeyPem(String name, PrivateKey key) throws IOException {
        String b64 = android.util.Base64.encodeToString(key.getEncoded(), android.util.Base64.NO_WRAP);
